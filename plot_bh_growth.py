...
bh_props_list = ['Times', 'ParticleIDs',
                 ...
                 'NumberOfRepositionings', 'NumberOfRepositionAttempts',
                 'NumberOfSwallows', 'NumberOfMergers', 'ViscosityFactors']

panels = ['Masses', 'AccretionRate', 'Speeds', 'Density',
          'Repositionings', 'RepositioningFractions']

yranges = {'Masses': [4.8, 8.0],
           'AccretionRates': [-8.9, 0],
           'ViscosityFactors': [-4, 0.05],
           'Speeds': [-0.9, 3.0],
           'Density': [4.0, 9.0],
           'Repositionings': [0, 100],
           'RepositioningFractions': [0, 1]}

xlabel = r'Time [Myr]'
ylabels = {'Masses': r'BH masses (log $m$ [M$_\odot$])',
           'AccretionRates': 'Accretion rates' '\n'
                             r'(log $\dot{m}$ [M$_\odot$/yr])',
           'ViscosityFactors': r'log Viscosity Factors',
           'Speeds': r'Gas speeds (log $v$ [km/s])',
           'Density': r'Gas density (log)',
           'Repositionings': r'Reposition numbers',
           'RepositioningFractions': r'Reposition fractions'}
...
def main():
    ...
    if args.plot_mass_range is not None:
        yranges['Masses'] = np.log10(np.array(args.plot_mass_range))
...
def process_sim(isim, args):
    ...
    args.catloc = f'{wdir}{args.bh_file}'
    ...
    if args.bh_bid is not None:
        ...
        if args.bh_mass_range is not None:
            zreds = hd.read_data(wdir + args.bh_file, 'Redshifts')
...
def process_bh(args, bh_data, ibh, isim):
    ...
    yranges['Repositionings'][1] = np.max(bh_data['NumberOfRepositionAttempts'])

    # Actual plots
    for iipanel, ipanel in panels:
    ...
    plt.show
    plt.savefig(f'{args.plot_prefix}_BH{ibh}.pdf', dpi=200)
    plt.close('all')
...
def plot_bh_panel(args, bh_data, plot_config, iipanel):
    ...
    draw_time_lines(plot_config['events'], yr)
    ...
    if panel == 'Masses':
        plt.plot(times,
                 ...
                 color='black')
        plt.plot(times, np.log10(bh_data['Masses'][ibh, inds] * conv_mass),
    ...
    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
        ...
        legend_item(ax, [0.03, 0.1], 0.06, 'Gas sound speed', color='goldenrod',
                    ls=':')
        legend_item(ax, [0.03, 0.1], 0.2, 'Gas bulk speed')

        if plot_bvel:
            vbh = np.log10(np.linalg.norm(bh_data['Velocities'][ibh, :, inds],
                                          axis=1))
            plt.plot(times, vbh, color='black', linestyle = '--')
            legend_item(ax, [0.03, 0.1], 0.13, 'BH velocity',
            ...
            legend_item(ax, [0.03, 0.1], 0.13, 'Gas circular speed',
                        color='cornflowerblue', ls='--')
            ...
            legend_item(ax, [0.03, 0.1], 0.13, 'Gas velocity dispersion',
                        color='cornflowerblue', ls='--')
    ...
    elif panel == 'Repositionings':
        plt.plot(times, bh_data['NumberOfRepositionings'][ibh, inds],
                 color='black')
        plt.plot(times, bh_data['NumberOfRepositionAttempts'][ibh, inds],
                 color='black', linestyle=':')
        legend_item(ax, [0.03, 0.1], 0.06, 'Actual repositionings')
        legend_item(ax, [0.03, 0.1], 0.13, 'Attempted repositionings',
                    ls=':', alpha=0.5)

    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 

    elif panel == 'RepositioningFractions':
        frac_repos, frac_att = get_repositioning_fractions(args, plot_config,
                                                           bh_data)
        plt.plot(times[inds], frac_repos[inds], color='black')
        plt.plot(times[inds], frac_att[inds], color='black', linestyle=':')
        legend_item(ax, [0.03, 0.1], 0.06, 'Actual repositionings')
        legend_item(ax, [0.03, 0.1], 0.06, 'Attempted repositionings',
                    ls=':', alpha=0.5)
    ...
    if iipanel == 0:
        ...
        # Code info
        legend_text(ax, 0.02, 0.08,
            f"{bh_data['CodeBranch']} ({bh_data['CodeRev']})")
        legend_text(ax, 0.02, 0.02, f"{bh_data['CodeDate']}")
        ...
        legend_text(ax, 0.03, 0.03, f"{bh_data['SimName']}")
...
def legend_item(ax, xr, y, text, alpha=1, ls='-', color='black'):
    ...
    plt.text(tr[0] + (tr[1]-tr[0]) * (xr[1] + 0.02),
             yr[0] + (yr[1]-yr[0]) * (y - 0.00),
             text, va='center', ha='left', alpha=alpha, color=color)
...
def find_event_times(bh_data, ibh):
    ...
    ind_repos = np.nonzero((bh_data['NumberOfRepositionings'][ibh, 1:] >
                            bh_data['NumberOfRepositionings'][ibh, :-1]) &
                           (bh_data['NumberOfRepositionings'][ibh, 1:] > 0))[0]

    if len(ind_repos) > 0:
        time_repos = bh_data['Times'][ind_repos]
    else:
        time_repos = None
...
def draw_time_lines(event_times, yr):
    ...
    if event_times['Repositionings'] is not None:
        for itime in event_times['Repositionings']:
...
def get_yaxis_viscosity(ax):
    ...
    ax2.set_ylim(yranges['ViscosityFactors'])
    ax2.set_ylabel(ylabels['ViscosityFactors'])
...
def get_repositioning_fractions(args, plot_config, bh_data):
    ...
    num_repos = bh_data['NumberOfRepositionings']
